...
# 1.The expectation of residual should be equal to 0. As we can see in the graph below, the red line is away 
#from line 0, so that the residual is biased. 2.The points are distributed randomly and the range of all 
#residuals is not too large.I would say the assumptions are not met well.
```
...
1. Give the equation of the regression line and the residual standard deviation of the regression.
  #regressional line:
    log(30000) = B + (0.008/0.01)*log(66) therefore B = 6.957229
    log(earnings) = 6.957229 + 0.8*log(height)
  #residual standard deviation:
   P(log(y)+log(1.1))=0.95
```{r}
    resi_sd <- log(1.1)/2
    resi_sd
```
2. Suppose the standard deviation of log heights is 5% in this population. What, then, is the $R^2$ of the regression model described here?
  Since $R^{2}=\frac{SSR}{SSTO}=1-\frac{SSE}{SSTO}$ and $SSE=VAR_{residual}*n^{2}$ and $SSR=VAR_{egression}*n^{2}$
```{r}
...
plot(breg1)
ggplot(breg1, aes(x=btystdave + tenured + age, y=courseevaluation)) + geom_point() + geom_smooth(method = "lm")
summary(breg1)
# From the P value we can tell that the "tenured" and "age" are not significant since their P value is much larger
#than 0.05. While the p value of "bytstdave" is well smaller than 0.05, therefore it's significant. The coefficient 
#of "btystdave" is 0.132830, which means it has positive correlation with the course evaluation, and the beautiful 
#a instructor, the higher evaluation score that instructor will get.
```
...
plot(breg2)
summary(breg2)
# The inputs are "btystdave" and "btystdml", predictors include all inputs plus the interaction between the two
#predictors. Both predictors are significant and "btystdave" has positive correlation while "btystdml" has 
...
What can you say about statistical significance of regression coefficient?
    *Since there are only 7 out of 100 z-scores are statistically significant, I would the regression coefficient is not significant.*
...
# By using the trditional way, the coefficient of education is the same value.
```
(b) The intercept for the simple regression in step 3 is 0.  Why is this the case?
  *The expectation of both residuals of "prreg1" and "prreg2" are 0. When we conduct the third step, we have $E(\epsilon _{y}) = E(\alpha *\epsilon _{x1}+\beta) = 0$, so that $\beta$ has to be zero.*
(c) In light of this procedure, is it reasonable to describe $B_1$ as the "effect of $X_1$ on $Y$ when the influence of $X_2,\cdots,X_k$ is removed from both $X_1$ and $Y$"?
  *I would say the statement is reasonable since we have removed all other predictors from the model.*
...
1. Using the Canadian occupational prestige data, calculate the partial correlation between prestige and education, controlling for income and percentage women.
  #Using the residuals from previous problem, we can get partial correlation 0.7362
```{r}
...
3. Imagine that educational researchers wish to assess the efficacy of a new program to improve the reading performance of children. To test the program, they recruit a group of children who are reading substantially vbelow grade level; after a year in the program, the researchers observe that the children, on average, have imporved their reading performance.  Why is this a weak research design?  How could it be improved?
  *This is a weak research design since the researchers only picked those students that had below average reading level, and this process would result in huge sampling bias. In order to improve the research design, researchers need to go back to the very beginning and conduct a random sampling withing the population.*
